...
fetch_hevy.py
-------------
Pulls workout data from the Hevy API and generates:
  1. A per-workout file (every sync with new data) → workout_reports/
  2. A cumulative weekly report (sync + weekly mode) → weekly_reports/

Modes:
  --mode weekly : Full weekly report (Sunday cron)
  --mode sync   : Check for new workouts, write per-workout file + update weekly report
...
def detect_session_type(workout_title):
    ...
    if any(k in t for k in VOLUME_KEYWORDS): return "volume"
    return "both"
...
def hevy_get(endpoint, params=None):
    ...
    resp = requests.get(
        f"{HEVY_BASE_URL}/{endpoint}", headers=headers, params=params, timeout=30
    )
...
def fmt_sets_detail(working_sets):
    ...
    for i, s in enumerate(working_sets, 1):
        w = s["weight_kg"]; r = s["reps"]; d = s.get("duration_s", 0)
        if w > 0 and r > 0:   parts.append(f"S{i}: {w}kg×{r}")
        elif w > 0 and d > 0: parts.append(f"S{i}: {w}kg×{d}s")
        elif r > 0:            parts.append(f"S{i}: BW×{r}")
        elif d > 0:            parts.append(f"S{i}: {d}s")
    ...
    for w in workouts:
        ...
        for ex in w.get("exercises", []):
            ...
            for s in ex.get("sets", []):
                ...
                if item["type"] == "warmup": warmups.append(item)
                elif item["type"] == "normal": working.append(item)
...
# ── Exercise section builder (shared by both report types) ─────────
def exercise_section(ex_names, section_title, grouped, week_number):
    ...
    for ex_name in ex_names:
        ...
        lines.append(f"\n#### {ex_name}{star}")
        if note: lines.append(f"> ⚠️ {note}\n")
        if not entries:
            target = get_target(ex_name, week_number, "both") or get_target(ex_name, week_number, "heavy")
            if target:
                t_str = fmt_target(target, reps_note)
                lines += ["| Session | Target | Actual | Status |",
                          "|---|---|---|---|",
                          f"| — | {t_str} | Not logged | ❌ Missing |"]
            continue
        lines += ["| Date | Session | Target | Actual | Status | All Sets |",
                  "|---|---|---|---|---|---|"]
        for entry in sorted(entries, key=lambda x: x["workout_date"]):
            ...
            t_str        = fmt_target(target, reps_note)
            actual_str   = f"{n_sets}×{best_r} @ {fmt_weight(best_w)}"
            if target:
                emoji, detail = assess(n_sets, best_w, best_r,
                                       target["sets"], target["weight"],
                                       target["reps"] if isinstance(target["reps"], int) else 0)
            else:
...
# ── NEW: per-workout report ────────────────────────────────────────
def generate_workout_report(workout, week_number):
    """One focused report for a single session — used for post-workout Claude check-in."""
    title        = workout.get("title", "Unknown")
    date         = workout.get("start_time", "")[:10]
    duration_min = round((workout.get("duration") or 0) / 60)
    session_type = detect_session_type(title)

    entries = extract_exercises([workout])
    grouped = group_by_exercise(entries)
    total_sets = sum(e["n_working"] for e in entries)

    # Planned exercises for this session type
    ...
    pull_back      = ["Pull Up (Weighted)", "Pull Up", "Chin Up", "Iso-Lateral Row (Machine)",
                      "Seated Cable Row - Bar Grip", "Lat Pulldown (Cable)"]
    ...
    # Detect which muscle groups appeared and build relevant sections
    t = title.lower()
    sections = []
    if any(k in t for k in ["push", "chest", "bench"]):
        sections += [
    ...
    if any(k in t for k in ["pull", "back", "bicep"]):
        sections += [
    ...
    if any(k in t for k in ["leg", "squat", "quad", "hamstring"]):
        sections += [
    ...
    # Always include core if any core exercises logged
    if any(ex in grouped for ex in core_ex):
        sections.append(exercise_section(core_ex, "CORE", grouped, week_number))

    # If we couldn't detect session type from title, show everything logged
    if not sections:
        all_planned = set(push_chest + push_shoulders + push_triceps + pull_back +
                          pull_rear + pull_biceps + legs_quads + legs_hams +
                          legs_calves + core_ex)
        for ex in grouped:
            if ex not in all_planned:
                sections.append(f"\n#### {ex} 📝\n> Not in programme target list\n")

    # Extra / unplanned exercises
    all_planned = set(push_chest + push_shoulders + push_triceps + pull_back +
                      pull_rear + pull_biceps + legs_quads + legs_hams +
                      legs_calves + core_ex)
    extras = [ex for ex in grouped if ex not in all_planned]
    extra_lines = []
    if extras:
    ...
    # Priority lift snapshot (only show ones relevant to this session)
    priority_rows = []
    priority_map = {
        "Bench Press (Barbell)":   ("Bench Press ⭐", "heavy"),
        "Pull Up (Weighted)":      ("Weighted Pull Up ⭐", "heavy"),
        "Leg Press (Machine)":     ("Leg Press ⭐", "both"),
        "Seated Leg Curl (Machine)": ("Seated Leg Curl ⭐", "both"),
    }
    for ex_name, (label, stype) in priority_map.items():
        if ex_name in grouped:
            e    = grouped[ex_name][0]
            t_   = get_target(ex_name, week_number, stype) or get_target(ex_name, week_number, "both")
            t_str= fmt_target(t_) if t_ else "—"
            actual = f"{e['n_working']}×{e['best_reps']} @ {fmt_weight(e['best_weight'])}"
            if t_:
                emoji, _ = assess(e["n_working"], e["best_weight"], e["best_reps"],
                                  t_["sets"], t_["weight"],
                                  t_["reps"] if isinstance(t_["reps"], int) else 0)
            else:
                emoji = "📝"
            priority_rows.append(f"| {label} | {t_str} | {actual} | {emoji} |")
    ...
    report = f"""# Session Report — {title}
**Date:** {date} | **Week {week_number} of 8** | **Duration:** {duration_min} min | **Working sets:** {total_sets}
...
def save_workout_report(report, workout, week_number):
    date  = workout.get("start_time", "")[:10]
    title = workout.get("title", "Unknown")
    safe  = title.replace(" ", "_").replace("/", "-")[:40]
    fname = f"{date}_{safe}_W{week_number}.md"
    path  = REPO_ROOT / "workout_reports" / fname
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(report, encoding="utf-8")
    print(f"Workout report saved: {fname}")
    return path, fname

# ── Weekly report (unchanged logic) ───────────────────────────────
def generate_report(workouts, week_number):
    ...
    def snap(ex, wk):
        ...
        if not e_list: return "❌ Not logged", "❌"
        best = max(e_list, key=lambda x: x["best_weight"])
        t    = get_target(ex, wk, best["session_type"])
        actual = f"{best['n_working']}×{best['best_reps']} @ {fmt_weight(best['best_weight'])}"
        if t:
            emoji, _ = assess(best["n_working"], best["best_weight"], best["best_reps"],
                               t["sets"], t["weight"],
                               t["reps"] if isinstance(t["reps"], int) else 0)
        else:
            emoji = "📝"
        return actual, emoji
    ...
    vol_status = ("✅ On target"         if 99 <= total_sets <= 135 else
                  "⚠️ High — watch recovery" if total_sets > 135 else
                  "❌ Below target")
    ...
    core_ex        = ["Plank", "Dead Hang", "Scapular Pull Ups"]
    all_planned    = set(push_chest + push_shoulders + push_triceps + pull_back +
                         pull_rear + pull_biceps + legs_quads + legs_hams + legs_calves + core_ex)
...
def run_sync():
    ...
    print(f"New workout(s) detected — writing {len(new)} per-workout report(s) + updating weekly")
    week = get_current_week()

    # Write a per-workout report for each new workout
    for workout in new:
        w_report = generate_workout_report(workout, week)
        save_workout_report(w_report, workout, week)

    # Regenerate the cumulative weekly report
    since        = datetime.now(timezone.utc) - timedelta(days=7)
